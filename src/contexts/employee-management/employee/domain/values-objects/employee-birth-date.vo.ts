...
function parseFlexibleDate(input: string | Date): Date {
  if (input instanceof Date) {
    if (isNaN(input.getTime())) throw new InvalidEmployeeException('La fecha de cumpleaños es inválida.');
    return input;
  }
  if (typeof input !== 'string') throw new InvalidEmployeeException('La fecha de cumpleaños debe ser string o Date.');
  ...
  if (isNaN(date.getTime())) {
    throw new InvalidEmployeeException('La fecha de cumpleaños no tiene un formato válido.');
  }
  return date;
}
...
export class EmployeeBirthDateVO extends ValueObject<Prop> {
  ...
  static create (value?: Date | string | null) {
    if (value === null || value === undefined) {
      return new EmployeeBirthDateVO({value: null});
    }
    const dateValue = parseFlexibleDate(value);
    return new EmployeeBirthDateVO({value: dateValue});
  }
