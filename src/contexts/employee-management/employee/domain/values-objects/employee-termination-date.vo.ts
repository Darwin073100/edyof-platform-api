...
function parseFlexibleDate(input: string | Date): Date {
  if (input instanceof Date) {
    if (isNaN(input.getTime())) throw new InvalidEmployeeException('La fecha de baja es inválida.');
    return input;
  }
  if (typeof input !== 'string') throw new InvalidEmployeeException('La fecha de baja debe ser string o Date.');
  ...
  if (isNaN(date.getTime())) {
    throw new InvalidEmployeeException('La fecha de baja no tiene un formato válido.');
  }
  return date;
}
...
export class EmployeeTerminationDateVO extends ValueObject<Prop> {
  ...
  static create (value?: Date | string | null) {
    if (value === null || value === undefined) {
      return new EmployeeTerminationDateVO({value: null});
    }
    const dateValue = parseFlexibleDate(value);
    return new EmployeeTerminationDateVO({value: dateValue});
  }
