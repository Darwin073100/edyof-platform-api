...
export class RegisterEstablishmentUseCase {
  ...
  public async execute(command: RegisterEstablishmentDto): Promise<EstablishmentEntity> {
    ...
    const newEstablishmentId = BigInt(Date.now()); // Simulación de ID

    // La entidad de dominio se crea con sus invariantes protegidas.
    const newEstablishment = EstablishmentEntity.create(newEstablishmentId, name);
    ...
    const domainEvents = newEstablishment.getAndClearEvents();
