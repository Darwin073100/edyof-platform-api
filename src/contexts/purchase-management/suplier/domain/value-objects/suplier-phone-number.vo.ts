...
export class SuplierPhoneNumberVO extends ValueObject<Prop> {
   ...
   */
  public static create(name: string): SuplierPhoneNumberVO {
    if (!name || name.trim().length === 0) {
      throw new InvalidSuplierException('El número de teléfono no puede estar vacío.'); // Un error de dominio apropiado
    }
    if (name.length > 25) { // Un ejemplo de límite, puedes ajustar
    ...
    return new SuplierPhoneNumberVO({ value:name });
  }
