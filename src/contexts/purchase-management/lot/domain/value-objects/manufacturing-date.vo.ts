...
function parseFlexibleDate(input: string | Date): Date {
  ...
  if (isNaN(date.getTime())) {
    throw new LotValidateException('La fecha de fabricación no tiene un formato válido.');
  }
  return date;
}
interface Prop{
...
export class ManufacturingDateVO extends ValueObject<Prop> {
  ...
  static create(value?: Date | string | null) {
    if (value === null || value === undefined) {
      return new ManufacturingDateVO({value: null});
    }
    const dateValue = parseFlexibleDate(value);
    return new ManufacturingDateVO({value: dateValue});
  }
