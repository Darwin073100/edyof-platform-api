...
export class InventoryItemMinStockBranchVO extends ValueObject<Prop> {
  ...
  public static create(name?: number|null): InventoryItemMinStockBranchVO {
    if (name && name < 0) { // Un ejemplo de límite, puedes ajustar
      throw new InvalidInventoryItemException('El stock minimo por sucursal no puede ser negativo.');
    }
    // Podrías añadir más validaciones: caracteres especiales, formato, etc.

    return new InventoryItemMinStockBranchVO({ value:name });
  }
